...
do_apply() {
  ...
  info "patchctl apply --upstream $short --auto-resolve-conflicts ${force_flag[*]:-} ${port_flag[*]:-}…"
  set +e
  "$PYTHON" maint/scripts/patchctl.py apply --upstream "$sha" --branch "$branch_name" --expect-version "$expect_v" --auto-resolve-conflicts ${force_flag[@]+"${force_flag[@]}"} ${port_flag[@]+"${port_flag[@]}"}
  local rc=$?
  set -e
