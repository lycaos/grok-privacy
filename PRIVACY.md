# Grok Privacy privacy model

Grok Privacy is a **VSCodium-style** community distribution of xAI Grok Build:
...
plus `GROK_TEST_ALLOW_UPDATE=1` can relax the vendor-update gate so local
...
4. **Repo change packaging** — `[repo_changes_dedup] enabled` defaults to
   `false`.
5. **Vendor auto-update** — hard-disabled. Grok Privacy never installs from
...
That is inference context, not Grok Privacy research upload. Prefer not opening
...
Channels the binary *can* touch, and how Grok Privacy treats them:
...
Grok Privacy locks **coding-data retention to opt-out** (no `/privacy opt-in`,
