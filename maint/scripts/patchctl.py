...
def attempt_auto_resolve(
    ...
    rerere (enabled on the failed am) may already have staged everything; what
    remains unmerged goes to the resolver command. Only unmerged files are ever
    written. Returns (record, "") on success or (None, reason) to fail closed.
    """
    ...
    if pending:
        if not entry.get("contracts"):
            return None, (
                f"patch declares no contracts; resolver disabled for "
                f"{patch_name} (recorded resolutions only)"
            )
        mail = (
            git(
    ...
    return {"patch": patch_name, "method": method, "files": files}, ""
...
def apply_series(
    ...
    *,
    ...
    for idx in range(start, len(series)):
        ...
        proc = git(
            [
                *(RERERE_GIT_FLAGS if auto_resolve else []),
                "am",
                "--3way",
                str(patch_file),
            ],
        ...
        if critical or not is_trailing_skippable(series, idx, meta):
            ...
            if auto_resolve:
                record, reason = attempt_auto_resolve(
                    root, patch_name, entry, files, report
                )
                ...
                if files:
                    # Partial resolver writes must not reach the maintainer:
                    # restore the conflicted state of every unmerged file.
            ...
            if keep_conflict:
                # Resume state only exists when the conflict is left in place:
                # after an abort there is nothing to continue, and a plain
                # re-run must stay possible.
...
def cmd_apply_continue(args: argparse.Namespace) -> int:
    ...
    if (git_dir(root) / "rebase-apply").exists():
        ...
        proc = git(
            [
                # Record the manual resolution so a later --auto-resolve-conflicts
                # run replays it without the resolver.
                *(RERERE_GIT_FLAGS if auto_resolve else []),
                "-c",
                "core.editor=true",
                "am",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    a.add_argument(
        ...
        action="store_true",
        help="On a critical-patch conflict, replay recorded resolutions and ask "
        "the resolver command (PATCHCTL_CONFLICT_RESOLVER_CMD, default "
        f"'{DEFAULT_RESOLVER_CMD}') for new ones; a resolution is accepted only "
        "if the patch's declared contracts pass on the applied tree",
    )
    a.add_argument(
        "--keep-conflict",
        action="store_true",
