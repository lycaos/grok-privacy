...
<h1>
  <picture>
    ...
    <img alt="Grok Privacy logo" src="docs/assets/gork-build-symbol-black.png" width="96">
  </picture>
  <br>
  Grok Privacy (<code>grok</code>)
</h1>

**Grok Privacy: the VSCodium-style community build of Grok Build — [research / product analytics hard-off](https://gist.github.com/cereblab/dc9a40bc26120f4540e4e09b75ffb547)**
...
![Grok Privacy TUI](docs/assets/gork-build-tui-screenshot.jpg)

**Grok Privacy is to [Grok Build](https://github.com/xai-org/grok-build) what
[VSCodium](https://github.com/VSCodium/vscodium) is to VS Code.**

This repository contains the Rust source for the `grok` CLI/TUI and agent
...
| | Grok Build (upstream) | **Grok Privacy** (this fork) |
|--|----------------------|---------------------------|
...
**Grok Privacy** re-ships that code with **privacy by construction**:
...
- Vendor auto-update is **hard-disabled**: Grok Privacy never installs from
...
product-analytics hard-offs. Grok Privacy does not add extra research packaging
on top.
...
```sh
cargo run -p xai-grok-pager-bin              # build + launch TUI (binary: grok)
cargo build -p xai-grok-pager-bin --release  # target/release/grok
cargo check -p xai-grok-pager-bin
```

Install the release binary somewhere on your `PATH` as `grok` (and optionally
...
```toml
# ~/.grok/config.toml — all of these are already the Grok Privacy defaults
[features]
telemetry = false
...
![Grok Privacy session — Made with Grok 4.5](docs/assets/made-with-grok-4.5.png)
...
Apache-2.0. Grok Privacy is an independent community distribution and is **not**
...
modifications are copyright the Grok Privacy contributors.
